...
import { RAW_SALES } from '../salesData';
...
function buildCustomers(): Record<string, CustomerProfile> {
  const customers: Record<string, CustomerProfile> = {};

  for (const s of RAW_SALES) {
    if (!customers[s.customer]) {
      customers[s.customer] = {
        name: s.customer, transactions: [], products: {},
...
const CustomerProfiles: React.FC = () => {
  const allCustomers = useMemo(() => buildCustomers(), []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 mb-2">Customer Profiles</h2>
